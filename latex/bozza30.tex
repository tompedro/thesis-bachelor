...
Consider the set of preparations given with a quantum system. Through the procedure described earlier, the equivalence classes of identical preparations are called \emph{states}, denoted by $\omega$. The expectation value $\omega(A) \in \mathbb{R}$ is obtained as the limit of sample means of replicated measurements of $A$ on identically prepared copies of the system. Since sample means are linear in the individual readings and intrinsically non-negative when evaluating squares of outcomes, we elevate these operational facts to defining axioms for the states.
	...
	\begin{remark}
		Definition \ref{def:state} guarantees by construction that every state evaluates non-negatively on the algebraic squares of observables. However, the converse implication, namely that state-positivity guarantees the existence of an exact square root within $\mathcal{O}$, fails. Establishing this strict equivalence requires endowing the observable set with a topological structure, such as norm completeness within a $C^*$-algebra. In this setting the continuous functional calculus ensures that such roots are always well defined.
	\end{remark}
...
states of Definition~\ref{def:state}, form a bounded set of real numbers,
...
\begin{definition}[Embedding of a JB algebra in a $C^*$-algebra]\label{def:jb-embedding}
	...
	\begin{enumerate}
		\item $A^* = A$ in $\A$, so that $\mathcal{O}$ is identified with a closed real subspace of the selfadjoint elements $\A_{sa}$ as per Definition \ref{def:selfadj-sector},
...
\begin{proposition}[Selfadjointness of the rescaled commutator]
...
\begin{proposition}[Recovery of the associative product]
	Let $\mathcal{O}$ be a JC algebra as in Definition \ref{def:jc-algebra}. For all $A,B\in\mathcal{O}$,$$AB=A\circ B+\tfrac{i\hbar}{2}\{A,B\}.$$
\end{proposition}
\begin{proof}
...
\begin{remark}
	...
	Definition~\ref{def:physical-system} in the next subsection.
\end{remark}
...
\begin{remark}
	The attainment proposition mathematically crystallizes the physical interpretation of the spectrum. Operationally, $\sigma(A)$ constitutes the precise set of values that a measurement of $A$ will yield with zero variance when the system is prepared in a suitable, dispersion-free state. For a generic state $\omega$, the expectation value $\omega(A)$ is not necessarily a sharp reading but rather a statistical average over the spectrum, thereby falling within the convex hull of $\sigma(A)$.
\end{remark}
...
\begin{definition}[Hermitian Inner Product]\label{def:hermitian-ip}
	Let $V$ be a complex vector space. A Hermitian inner product on $V$ is a map $\langle\cdot,\cdot\rangle: V\times V\to \C$ satisfying the following properties for all $v,w,w'\in V$ and $\alpha,\beta\in\C$.
	\begin{enumerate}
		\item $\langle v,v\rangle\geq 0$.
		\item $\langle v,v\rangle=0$ if and only if $v=0$.
		\item $\langle v,\alpha w+\beta w'\rangle=\alpha\langle v,w\rangle+\beta\langle v,w'\rangle$.
...
\begin{proposition}[The Inner Product Induces a Norm]\label{prop:induced-norm}
	Let $V$ be a complex vector space equipped with a Hermitian inner product as per Definition~\ref{def:hermitian-ip}. The map $\|\cdot\|: V\to\mathbb{R}$ defined by $\|v\|=\sqrt{\langle v,v\rangle}$ is a norm on $V$.
\end{proposition}

\noindent A proof of this statement can be found in \cite[Proposition 3.3]{morettiSpectralTheoryQuantum}. 
\begin{example}
	The space $\C^n$ with $\langle v,w\rangle=\sum_{k=1}^n\bar v_kw_k$ is a finite-dimensional pre-Hilbert space. The space $\ell^2$ of square-summable sequences, with the analogous inner product, is an infinite-dimensional example used throughout this work.
\end{example}
...
\begin{example}\label{ex:c01-incomplete}
	The space $C([0,1])$ with $\langle f,h\rangle=\int_0^1\overline{f(x)}h(x)\,dx$ is a pre-Hilbert space that is not complete, since a sequence of continuous functions approximating a jump discontinuity is Cauchy without a continuous limit. Both $\C^n$ and $\ell^2$ are complete, hence genuine Hilbert spaces.
\end{example}
...
\begin{definition}[Dense Subset]\label{def:dense}
	Let $V$ be a normed space as per Definition \ref{def:normed-space}. A subset $W\subseteq V$ is dense in $V$ if for every $v\in V$ and for every $\varepsilon>0$ there exists $w\in W$ with $\|v-w\|<\varepsilon$.
\end{definition}
...
\begin{definition}[Linear Isometry]\label{def:isometry}
	Let $V,V'$ be normed spaces as per Definition \ref{def:normed-space}. A linear map $J: V\to V'$ is a linear isometry if $\|Jv\|_{V'}=\|v\|_V$ for every $v\in V$.
\end{definition}
...
\begin{theorem}[Completion of a Pre-Hilbert Space]\label{thm:completion}
	...
	\begin{equation}
		\langle Jv,Jw\rangle_{\H}=\langle v,w\rangle_V,
	\end{equation}
	for all $v,w\in V$.
\end{theorem}
...
\begin{definition}[Operator Norm and Boundedness]\label{def:op-norm-bdd}
	Let $X,Y$ be normed spaces as in Definition \ref{def:normed-space} and let $T\in\mathcal{L}(X,Y)$ with $\mathcal{L}(X,Y)$ the space of linear maps between $X$ and $Y$. The operator $T$ is bounded if there exists $K\geq 0$ such that
	\begin{equation}
		\|Tv\|_Y\leq K\|v\|_X,
	\end{equation}
...
A complete proof can be found in \cite[Proposition 2.47]{morettiSpectralTheoryQuantum}. Two vectors in a Hilbert space are orthogonal if their inner product vanishes. Every closed subspace splits $\H$ into two orthogonal pieces, which yields the Riesz representation theorem for continuous linear functionals. 
...
\begin{definition}[Orthogonal Complement]\label{def:orth-complement}
	...
	\begin{equation}
		K^{\perp}=\{v\in\H\mid\langle v,w\rangle=0\text{ for every }w\in K\}.
	\end{equation}
\end{definition}
...
\begin{example}\label{ex:riesz-l2}
	On $\H=\ell^2$, fix $a\in\ell^2$ and define $Fw=\sum_{k=1}^{\infty}\bar a_kw_k$. Then $F$ is a bounded linear functional with $\|F\|=\|a\|$, and $Fw=\langle a,w\rangle$, so the Riesz vector representing $F$ is $a$ itself.
\end{example}
...
\begin{proposition}[Adjoint of Bounded Operators]\label{prop:adjoint-bdd}
	...
	\begin{equation}
		\langle\varphi,T\psi\rangle_{\H_2}=\langle T^{*}\varphi,\psi\rangle_{\H_1},
	\end{equation}
...
\begin{definition}[Special Classes of Bounded Operators]\label{def:special-classes}
	...
	\begin{enumerate}
		...
		\item positive, written $T\geq 0$, if $\langle\psi,T\psi\rangle\geq 0$ for every $\psi\in\H$.
	\end{enumerate}
\end{definition}
...
\begin{definition}[Adjoint of Unbounded Operators]\label{def:adjoint-unbdd}
	...
	\begin{equation}
		D(T^{*})=\{\varphi\in\H\mid\exists\,\omega_\varphi\in\H\text{ such that }\langle\varphi,T\psi\rangle=\langle\omega_\varphi,\psi\rangle \forall \psi\in D(T)\},
	\end{equation}
...
\begin{definition}[Hermitian Operator]\label{def:hermitian-op}
	...
	\begin{equation}
		\langle\varphi,T\psi\rangle=\langle T\varphi,\psi\rangle,
	\end{equation}
