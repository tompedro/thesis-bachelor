...
We can start by giving the most basic definitions that will be used throughout all the thesis. One of the most important one is the smooth manifold, but first we need to understand what a manifold is and why it is so important in physics.
\begin{definition}[Topological manifold]
	A topological manifold of dimension $n$ is a topological space $M$ that is Hausdorff, second countable, and locally Euclidean of dimension $n$: every point $p \in M$ has an open neighborhood homeomorphic to an open subset of $\mathbb{R}^n$. \cite{abrahamFoundationsMechanics2008}
\end{definition}

\begin{definition}[Chart, atlas]
	A chart on $M$ is a pair $(U,\varphi)$, with $U \subseteq M$ open and $\varphi: U \to \varphi(U) \subseteq \mathbb{R}^n$ a bijection onto an open subset. Two charts $(U,\varphi)$, $(V,\psi)$ are $C^\infty$-compatible if the transition map
	\[
	\psi \circ \varphi^{-1} : \varphi(U \cap V) \to \psi(U \cap V)
	\]
	is a $C^\infty$-diffeomorphism whenever $U \cap V \neq \emptyset$. An atlas is a collection of pairwise compatible charts covering $M$. The union of all the atlases on M is the maximal atlas.
\end{definition}
...
\begin{definition}[Smooth map, diffeomorphism]
	...
	\]
	is $C^\infty$ in the usual sense on $\mathbb{R}^m$. $f$ is smooth if smooth at every point. $f$ is a diffeomorphism if it is a smooth bijection with smooth inverse. We write $C^\infty(M) := C^\infty(M,\mathbb{R})$.
\end{definition}
\medspace
...
\begin{definition}[Tangent vector, tangent space]
	Fix $p \in M$. A tangent vector at $p$ is a linear map $v : C^\infty(M) \to \mathbb{R}$ satisfying the Leibniz rule
	\[
...
\begin{remark}
	Equivalently, a curve $\gamma:(-\varepsilon,\varepsilon)\to M$ with $\gamma(0)=p$ induces the derivation $v_\gamma(f) := \frac{d}{dt}\Big|_{t=0} f(\gamma(t))$, and every $v \in T_pM$ can be derived in this way. It can be proved that two curves induce the same tangent vector iff their local representatives agree to first order at $p$; this equivalent identification $T_pM \cong \{\text{curves through } p\}/\!\sim$ is used later (e.g.\ for the flows $\phi^H_t$).
\end{remark}

Sometimes it is convenient to switch to a notation more explicit than the global description really implicit used most of the times, for that we can write our objects on manifolds in coordinates.

\begin{definition}[Coordinate basis]
...
\begin{definition}[Vector field]
	A vector field on $M$ is a called a smooth section of $\pi: TM \to M$, if it is a smooth map $X: M \to TM$ with $\pi \circ X = \mathrm{id}_M$; equivalently, $p \mapsto X_p \in T_pM$ smooth in $p$. Equivalently again, $X$ is a derivation of the $\mathbb{R}$-algebra $C^\infty(M)$: linear, with $X(fg) = f\,X(g) + g\,X(f)$. We denote by $\mathfrak{X}(M)$ the $C^\infty(M)$-module of vector fields on $M$.
\end{definition}
...
\begin{definition}[Pushforward / Differential]
...
\begin{definition}[Flow of a Vector Field]
	Let $X$ be a smooth vector field on a smooth manifold $M$. An \textit{integral curve} of $X$ is a smooth curve $\gamma: I \to M$, where $I \subset \mathbb{R}$ is an open interval, such that $\gamma'(t) = X(\gamma(t))$ for all $t \in I$.
	
	The \textit{local flow} (or simply \textit{flow}) of $X$ is a smooth map $\theta: \mathcal{D} \to M$, where $\mathcal{D} \subset \mathbb{R} \times M$ is an open neighborhood of $\{0\} \times M$ known as the \textit{flow domain}, satisfying the following properties:
	\begin{enumerate}
		\item For each $p \in M$, the slice $\mathcal{D}^{(p)} = \{t \in \mathbb{R} : (t, p) \in \mathcal{D}\}$ is an open interval containing $0$, and the curve $\theta^{(p)}: \mathcal{D}^{(p)} \to M$ defined by $\theta^{(p)}(t) = \theta(t, p)$ is the unique maximal integral curve of $X$ satisfying the initial condition $\theta(0, p) = p$. That is,
		\[
		...
		\]
	\end{enumerate}
	If $\mathcal{D} = \mathbb{R} \times M$, the vector field $X$ is called \textit{complete}, and the maps $\theta_t: M \to M$ constitute a global one-parameter group of diffeomorphisms.
\end{definition}
...
\begin{remark}
	...
	structure via the commutator $[a,b]:=ab-ba$. It satisfy trivially the antisymmetry and Jacobi identity via:
	\[
...
\begin{example}\label{ex:su-n}
	Another example is $\mathfrak{su}(n)$. It consist of the $M(n, \C)$ matrices which are traceless and anti-hermitian, this makes the dimension of the resulting Lie algebra $n^2 -1$. In two dimensions, the basis of $\mathfrak{su}(2)$, consists of the Pauli operators $\sigma_i$ which $i = x,y,z$ which are used for representing the spin of a fermion in quantum physics. In three dimensions, the basis is formed by the eight Gell-Mann matrices $\lambda_a$ used in quantum chromodynamics.
\end{example}

Now, let us recall some basic concepts and fundamentals in the Lie theory. 
In general, an \textit{homomorphism} is an application between the same mathematical structures that preserves the inner operations. When, in particular, the structure is a Lie algebra we can use the following definition.

%So, in the particular case in which we are working on, we can use the following definition.
...
\begin{definition}[Lie Group]\label{def:lie-group}
	A Lie Groups is a smooth manifold G together with a smooth map from $G \cross G \to G$ that makes G into a group and such that the inverse map $g \to g^{-1}$ is a smooth map $G \to G$.
\end{definition}
...
\begin{definition}[Homomorphism of Lie Groups]\label{def:lie-grhomo}
	A map $F: G \to H$ where G and H are Lie groups is called an homomorphism if it is a smooth map and an homomorphism between groups.
\end{definition}

Let's look now at the tangent space of a Lie group. We can define an operation $L_g : G \to G$ by $L_g(h) = g \circ h$ called the \textit{"left multiplication"} which is smooth. So the differential $dL_g$ at the point $h$ will be a linear map from $T_h(G)$ to $T_{gh}(G)$.

Now we can take a vector field X on G, and that is called \textit{left-invariant} if
\[
...
\begin{proposition}[Completeness of left-invariant vector fields]\label{prop:complete}
	Let $X^v \in \mathfrak{X}(G)$ be the left-invariant vector field associated to $v \in \mathfrak{g} = T_eG$. Then $X^v$ is complete: its maximal integral curve through $e$ is defined for all $t \in \mathbb{R}$.
\end{proposition}

\begin{proposition}[One-parameter subgroup]\label{prop:one-param}
	Let $\gamma_v:\mathbb{R}\to G$ be the integral curve of $X^v$ through $e$. Then
	\[
	\gamma_v(t+s) = \gamma_v(t)\,\gamma_v(s) \qquad \forall\, t,s \in \mathbb{R},
	\]
	$\gamma_v$ is a Lie group homomorphism $\mathbb{R} \to G$ (a \emph{one-parameter subgroup}).
\end{proposition}
...
\begin{proposition}[Basic properties of $\exp$]\label{prop:exp-props}
	...
	\begin{enumerate}
		\item $\exp(tv) = \gamma_v(t)$ for all $t\in\mathbb{R}$ (from $\gamma_{tv}(s)=\gamma_v(ts)$).
...
\begin{remark}
	Global surjectivity of $\exp$ is not automatic in general (fails e.g.\ for $SL(2,\mathbb{R})$), but holds for $G$ compact and connected — which covers every group used in this thesis ($SU(2)$, $U(n)$).
\end{remark}

\begin{remark}[Matrix Lie groups]
	For $G \subseteq GL(n,\mathbb{C})$ a closed subgroup $\mathfrak g \subset \mathfrak{gl}(n,\mathbb{C}) = M_n(\mathbb{C})$ is identified with a linear subspace of matrices via $T_eG \subset T_eGL(n,\mathbb{C}) \cong M_n(\mathbb{C})$, and left multiplication is literally matrix multiplication: $L_g(h) = gh$, so $dL_g(v) = gv$ for $v \in \mathfrak g$. The left-invariant vector field associated to $v$ is therefore $X^v_g = gv$, and its integral curve through $e$ solves the linear matrix ODE
	\[
	\dot\gamma(t) = \gamma(t)\, v, \qquad \gamma(0) = I,
	\]
	...
	\[
	\mathfrak g = \{v \in M_n(\mathbb{C}) : e^{tv} \in G \ \forall t \in \mathbb{R}\},
	\]
...
\begin{example}[$\mathfrak{su}(2)$ from $SU(2)$ via the matrix exponential]
	Let $G = SU(2) = \{g \in GL(2,\mathbb{C}) : g^\dagger g = I,\ \det g = 1\}$. Differentiating both defining conditions along $\gamma(t) = e^{tv}$, $\gamma(0)=I$, gives the linearized conditions on $v$:
	\[
	...
	\[
	\mathfrak{su}(2) = \{v \in M_2(\mathbb{C}) : v^\dagger = -v,\ \operatorname{tr}(v) = 0\},
	\]
	a $3$-dimensional real vector space.
	
	Take $v = -\tfrac{i\theta}{2}\sigma_3$, $\theta \in \mathbb{R}$, $\sigma_3 = \mathrm{diag}(1,-1)$. Since $\sigma_3^2 = I$, the series splits into even and odd powers:
	\[
	\exp(v) = e^{-i\theta\sigma_3/2} = \cos\!\left(\frac{\theta}{2}\right) I - i\sin\!\left(\frac{\theta}{2}\right)\sigma_3
...
The previously presented concepts can be used to state the definition of a representation (or \textit{rep}). Given that we will use the following applied to various mathematical objects, we decided to first show the general definition on groups but it is clearly valid for Lie groups and Lie algebras with some modifies. \cite{hallLieGroupsLie2003}

\begin{definition}[Representation of a group]\label{def:group-rep}
	Let G be a group and V a vector space over $\K$. A representation of G on V is an homomorphism between groups:
	\[
		\rho : G \to GL(V),
	\]
...
\begin{definition}[Representation of a Lie Algebra]\label{def:lie-rep}
	A representation of a Lie Algebra $\mathfrak{g}$ is an homomorphism $\phi: \mathfrak{g} \to \mathfrak{gl}(V)$ where V is a $\K$-vector space.
\end{definition}
...
\begin{definition}[Faithful Representation]\label{def:rep-faith}
	...
	\[
		\ker{\rho} = \{g \in G | \quad \rho(g) = \Id_V\} = \{e\},
	\]
	where $e$ is the identity element of the group.
\end{definition}

\begin{remark}
	This definition is referring to the representations which don't lose the informations encoded in the group G. All the different elements of the group are being mapped into a different application.
\end{remark}
...
\begin{definition}[Irreducibility of a representation] \label{def:rep-irreps}
	...
	\begin{itemize}
	 \item A subspace W of V is called invariant if $(\pi(g))(w) \in W \quad \forall w \in W , \forall a \in G$. 
...
Thanks to that definition we can now state one of the most famous and important lemmas in representation theory. It

\begin{lemma}[Schur's Lemma]\label{lem:schur}
	Let $\pi: G \to GL(V)$ be a finite-dimensional \emph{irrep}  over $\mathbb{C}$.
	\begin{enumerate}
		\item If $T: V \to V$ is a linear map commuting with $\pi(g)$ for every $g$, then $T = \lambda \, \mathrm{id}_V$ for some $\lambda \in \mathbb{C}$.
...
\begin{example}[Schur's Lemma on $\mathfrak{su}(2)$]
	...
	By Schur's Lemma (part 1), any operator commuting with an irreducible representation is a scalar multiple of the identity:
	\[
...
\begin{proposition}\label{prop:skew-normal-form-manifold}
	...
	\end{pmatrix},
	\]
	where $I$ is the $n_p \times n_p$ identity matrix and the bottom-right block has size $(d-2n_p)\times(d-2n_p)$, $d := \dim T_pM$.
\end{proposition}
...
Finally, we can state the next important definition that encodes all the structure to derive Hamiltonian mechanics in the clearest and natural way possible.
...
\begin{proposition}
	If $(M,\omega)$ and $(N,\rho)$ be symplectic manifolds and $F:M \to N$ symplectic then F is volume preserving and $det_{(\Omega_\omega, \Omega_\rho)} F = 1$ and F is a local diffeomorphism.
\end{proposition}

The next theorem can be really obscure but it is a key result that we cannot omit in this introduction to symplectic geometry. The problem we want to solve is the following: Imagine you need to construct a 1-form on a manifold M, so you would need a rule to give any tangent vector a real number. If M is already a cotangent bundle of another manifold Q (that's the case of the phase space in Hamiltonian mechanics), this is really helping because there is a natural way to accomplish this.

Every point in M so it's a 1-form, viewed in the perspective of Q, let's call it $\alpha_q$. If we switch in the perspective of Q, now we can define the tangent vector at the point $\alpha_q$, let's call it $w_{\alpha_q}$. The natural way to create a 1-form now is to use $\alpha_q$ as a form defined on tangent vectors to Q. So we take $w_{\alpha_q}$, use a pushforward to move it to the tangent bundle of Q and finally pairing it with $\alpha_q$. This is what the \textit{canonical form} $\theta_0$ does.
\begin{theorem}[Canonical forms]\label{thm:canon}
	...
	\[
		\theta_{\alpha_q} : T_{\alpha_q}M \in \R \qquad w_{\alpha_q} \mapsto \alpha_q(d \tau^*_Q(w_{\alpha_q}))
	\] 
	and the map $\theta_0: \alpha_q \in \theta_{\alpha_q}$. In this case $\theta_0 \in \Omega^1(M)$ and $\omega_0=-d\theta_0$ is a symplectic form on M. The forms $\theta_0$ and $\omega_0$ are called the canonical forms on M.
\end{theorem}
...
\begin{theorem}[Invariance of the canonical forms]\label{thm:inv-canon}
	...
	\]
	Then $T^*f$ is symplectic and if I use the lift to pullback the canonical form $(T^*f)^*\theta_0 = \theta_0$ it comes right back.
\end{theorem}
...
\begin{proposition}[Hamilton Equations]\label{prop:ham-eq}
...
where $J = \begin{pmatrix} 0 & I \\ -I & 0 \end{pmatrix}$. Thus $(q(t),p(t))$ is an integral curve of $X_H$ iff Hamilton's equations hold:
\[
...
The next basic fact about Hamiltonian systems is that their flows consist of canonical transformations.
