...
It descends from Proposition \ref{prop:smooth-tm} that for $TM$ a coordinate basis $(U,\varphi=(x^1,\dots,x^n))$ on $M$ induces a counterpart on $\pi^{-1}(U)$ via $v = v^i \partial_i|_p \mapsto (x^1(p),\dots,x^n(p),v^1,\dots,v^n)$. The same construction can be extended to $T^*M$.
...
\textit{Note: In this chapter, $\K$ denotes an arbitrary commutative field with characteristic zero, following \cite{humphreysIntroductionLieAlgebras2010}.}
...
\begin{corollary}\label{cor:omega-prop}
	...
	\begin{itemize}
		\item $d$ must be even: a skew-symmetric non-degenerate $2$-form can exist only on an even-dimensional manifold.
