\epigraph{The reader will find no figures in this work. The methods which I set forth do not require either constructions or geometrical or mechanical reasonings: but only algebraic operations, subject to a regular and uniform rule of procedure.}{Giuseppe Luigi Lagrangia, Preface to Mécanique Analytique \cite{lagrangeMecaniqueAnalytique2009}}

\noindent This chapter establishes the geometric and algebraic language of classical mechanics in an invariant form. Moving beyond the traditional Newtonian approach, we adopt a coordinate-free framework on smooth manifolds. The logical sequence of the chapter is as follows: First, we introduce Lie algebras, relying on Humphreys \cite{humphreysIntroductionLieAlgebras2010} and Lee \cite{leeIntroductionSmoothManifolds2013}, to provide the formal infrastructure for the algebraic brackets. Second, using the foundations of symplectic geometry, following Abraham and Marsden \cite{abrahamFoundationsMechanics2008}, we identify $C^\infty(M)$ as the natural space for observables. In this context, Poisson brackets endow the space with a Lie algebra structure. Finally, by applying the Gelfand theorem, we recognize $C_0(M)$ as a commutative $C^*$-algebra. This leads to our primary structural result, namely classical mechanics formally coincides with the theory of commutative $C^*$-algebras.

\section{A Brief Introduction to Differential Geometry}

The geometric formulation of classical mechanics replaces the traditional Euclidean space with a coordinate-free framework on smooth manifolds. This abstraction is physically justified by holonomic constraints, which deform the flat configuration space and remove any global vector space structure. This section introduces the topological and differential tools required to extend ordinary calculus to such curved spaces.
...
In physics, configuration spaces, the space formed by all the possible spatial configurations of a system of particles, provide the most natural examples of such topological spaces. For instance, a simple pendulum constrained to move in a plane has the unit circle $\mathbb{S}^1$ as its configuration space, while the configurations of a free rigid body, in a frame of reference comoving with the centre of mass, are described by the special orthogonal group $SO(3)$. Neither of these spaces is globally homeomorphic to a Euclidean space. Hence, to perform calculus, a way of circumnavigate this consist of covering the manifold with open sets that locally resemble flat space.

\begin{definition}[Chart, atlas]\label{def:chart-atlas}
	A chart on $M$ is a pair $(U,\varphi)$, with $U \subseteq M$ open and $\varphi: U \to \varphi(U) \subseteq \R^n$ a homeomorphism. Two charts $(U,\varphi)$, $(V,\psi)$ are $C^\infty$-compatible if the transition map
	\[
...
\begin{remark}
	An equivalent formulation of Definition \ref{def:chart-atlas} could have been stated using $\varphi \circ \psi^{-1}$ that is a diffeomorphism being the inverse of $\psi \circ \varphi^{-1}$. 
\end{remark}
...
\begin{definition}[Smooth map, diffeomorphism]\label{def:diffeo}
	...
	\]
	is $C^\infty$ in the Euclidean topology. A function $f$ is smooth if it is such at every point, while it is a diffeomorphism if it is a smooth bijection with smooth inverse. We write $C^\infty(M) := C^\infty(M;\R)$.
\end{definition}
\medskip
...
\begin{definition}[Tangent vector]\label{def:tan-vector}
	...
	\]
	The set of these tangent vectors is the tangent space at $p \in M$ denoted as $T_pM$.
\end{definition}
...
Most of the times it is convenient to write statements locally on a specific chart. This allows one to use the rules of calculus on manifolds in order to simplify computations.

\begin{definition}[Coordinate basis]\label{def:coord}
	Let $(U,\varphi)$ be a chart around $p \in M$, an n-dimensional smooth manifold, with $\varphi : U \to V \subseteq \R^n$ a homeomorphism. Let $r^i : \R^n \to \R$ with $i=1,...,n$ be the standard coordinates of $\R^n$. Define the coordinate basis as
	\[
...
\begin{proposition}
	A coordinate basis as in Definition \ref{def:coord}, forms a basis of $T_pM$. Hence $\dim T_pM = n$, and $v = v^i\, \partial_i |_p$ using the Einstein summation convention.
\end{proposition}

A proof of this proposition can be found in \cite[Proposition 3.15]{leeIntroductionSmoothManifolds2013}. We can now assemble the tangent spaces at every point of $M$ into a single geometric object, endowed with its own smooth structure. In literature, it is used as the natural stage for the Lagrangian description of a mechanical system.
...
A single tangent vector specifies a generalized velocity at a specific configuration. To model global dynamical systems, we must assign smoothly a velocity vector to every point of the manifold, which leads to the notion of a vector field.
...
\begin{remark}\label{def:dual-basis}
...
\noindent The same construction that produced the tangent space from the individual tangent spaces can be repeated for their duals.
...
\begin{definition}[Cotangent space]\label{def:cot-space}
	...
	\]
	endowed with the canonical projection $\pi: T^*M \to M$.
...
The next proposition follows from the Vector Bundle Construction Lemma \cite[Lemma 10.6]{leeIntroductionSmoothManifolds2013} that will not be proved here. In short, it states that given a surjective local homeomorphism $\pi : X \to Y$ from a topological space $X$ and a smooth manifold $Y$, there exists a unique smooth structure on X, such that $\pi$ becomes a local diffeomorphism. 
...
It descends from Proposition \ref{prop:smooth-tm} that for $TM$ a coordinate basis $(U,\varphi=(x^1,\dots,x^n))$ on $M$ induces a counterpart on $\pi^{-1}(U)$ via $v = v^i \partial_i|_p \mapsto (x^1(p),\dots,x^n(p),v^1,\dots,v^n)$. A complete proof of this statement is shown in \cite[Proposition 3.18]{leeIntroductionSmoothManifolds2013}. The same construction can be extended to $T^*M$ and it can be found in \cite[Proposition 11.9]{leeIntroductionSmoothManifolds2013}.
...
With the structure of manifolds, tangent spaces, and the algebra of differential forms in place, we introduce the operators that act on them. In the geometric theory of mechanics, these operators map physical quantities between different spaces, through symmetries or coordinate transformations and generate the dynamical equations. The pushforward maps velocity vectors, its dual, the pullback, acts on configuration constraints and momenta.

First we see that there is a unique way to compute the product between vectors and covectors.
...
While the interior product acts as an algebraic contraction that reduces the degree of a form, we conversely require an operation to build higher-degree forms from lower-degree ones. To construct the full exterior algebra, we introduce an intrinsic product that combines a $k$ and an $l$-covector into a $(k+l)$-covector, preserving the alternating property.

\begin{definition}[Wedge product]\label{def:wedge-product}
	Let $M$ be a smooth manifold, and let $\omega \in \Lambda^k(T_p^*M)$ and $\eta \in \Lambda^l(T_p^*M)$ as per Definition in \ref{def:k-covector}. Their \emph{wedge product}, or exterior product, is the alternating $(k+l)$-covector $\omega \wedge \eta \in \Lambda^{k+l}(T_p^*M)$ defined by its evaluation on tangent vectors $v_1, \dots, v_{k+l} \in T_pM$:
	\[
...
\noindent This algebraic operation naturally extends pointwise to smooth sections.
...
\begin{definition}[Pushforward / Differential]\label{def:pushforward}
	...
	\[
		F_*: T_pM \to T_{F(p)}N,
	\]
	such that, for any tangent vector $X \in T_pM$ and for any smooth function $f \in C^\infty(N)$
	\[
	(F_*X)(f) := X(f \circ F).
	\]
...
\begin{remark}
	We can state an alternative of Definition \ref{def:pushforward} using the geometric description of tangent vectors as equivalence classes of curves. Let $\gamma \in C^\infty( (-\epsilon, \epsilon); M)$ such that $\gamma(0) = p$ and $\gamma'(0) = v \in T_pM$. Then the pushforward $F_*v$ is the velocity vector of the curve $F \circ \gamma$ at $t = 0$:
	\[
	F_*v = \frac{d}{dt}\Big|_{t=0} (F \circ \gamma)(t).
	\]
\end{remark}

The differential of a function yields the linear approximation of a scalar observable. To build the geometric machinery of classical mechanics, in particular the canonical symplectic form on the cotangent space, we need to extend this operation to higher-degree forms. The exterior derivative serves the job.

\begin{definition}[Exterior derivative]\label{def:ext-der}
...
\begin{remark}
	It can be proved that a family of mappings as in Definition \ref{def:ext-der} exist and it is unique.
\end{remark}
...
\begin{definition}[Pullback]\label{def:pullback}
	...
	defined as follows.
	\begin{enumerate}
		\item If $k = 0$, given $f \in C^\infty(N) \equiv \Omega^0(N)$, the pullback $F^*f \in C^\infty(M)$ is defined by precomposition:
		\[
		F^*f = f \circ F.
		\]
		
		\item If $k \geq 1$, for $\Lambda \in \Omega^k(N)$, the pullback $F^*\Lambda \in \Omega^k(M)$ is defined pointwise. For each $p \in M$ and all tangent vectors $X_1, \dots, X_k \in T_pM$,
		\[
...
\begin{remark}
	The case $k = 1$ recovers the pullback of covectors: for $\omega \in T_{F(p)}^*N$ and $v \in T_pM$,
	\[
	(F^*\omega)(v) = \omega(F_*v).
	\]
\end{remark}

Having described how differential forms and covectors transform under smooth maps, we now turn to the dynamical content of a vector field. In fact, the latter generates, through its integral curves, a continuous family of diffeomorphisms of the manifold. This flow is the fundamental geometric object underlying the notion of time evolution, and it will later allow us to identify the trajectories of a Hamiltonian system with the flow of the corresponding Hamiltonian vector field.
...
\begin{definition}[Flow of a Vector Field]\label{def:flow}
	...
	\noindent The \textit{(local) flow} of $X$ is a smooth map $\theta: \mathcal{D} \to M$, where $\mathcal{D} \subset \R \times M$ is an open neighborhood of $\{0\} \times M$ known as the \textit{flow domain}, satisfying the following properties
	\begin{enumerate}
...
\begin{proposition}
	Let $M$ be a smooth manifold and let $X$ be a vector flow with $\theta: \mathcal{D} \to M$ its flow as in Definition \ref{def:flow}. If $\mathcal{D} = \R \times M$, the vector field $X$ is called \textit{complete} and the maps $\theta_t: M \to M$ constitute a global one-parameter group of diffeomorphisms.
\end{proposition}

A proof of this statement can be constructed merging \cite[Proposition 9.7]{leeIntroductionSmoothManifolds2013} and \cite[Theorem 9.12]{leeIntroductionSmoothManifolds2013}. The concept of flow translates the analytical problem of solving systems of differential equations into the geometric action of a one-parameter group of diffeomorphisms. In the context of dynamics, if $X$ is the Hamiltonian vector field governing a mechanical system, its integral curves represent the physical trajectories in phase space, and the associated flow $\theta_t$ corresponds to the time evolution operator, which inherently preserves the symplectic structure. This will be analysed later in \ref{sec:1-ham}
...
\begin{theorem}[Cartan's Magic Formula]\label{thm:cartan}
	...
	\begin{equation}
		\mathscr{L}_V\omega = \iota_V(d\omega) + d(\iota_V\omega).\label{eq:cartan}
	\end{equation}
\end{theorem}
...
In this section we develop the algebraic component of classical mechanics. We first introduce Lie algebras abstractly. We then specialize to Lie groups, smooth manifolds carrying a compatible group structure, and show how every Lie group determines a Lie algebra through its left-invariant vector fields, with the exponential map provides the bridge back from the algebra to the group. Finally, we introduce the notion of a representation, which realizes the abstract elements of a Lie algebra or Lie group as concrete linear operators on a vector space. This last step is what will allow us, in later chapters, to identify the continuous symmetries of a quantum system with observables that can actually be measured.

\noindent \textit{Note that in this chapter, $\K$ denotes an arbitrary commutative field with characteristic zero, following \cite{humphreysIntroductionLieAlgebras2010}.}
...
To benefit from the algebraic structure of Lie algebras within the framework of differential geometry, we must introduce manifolds equipped with compatible smooth group operations. In order to canonically endow the tangent space at the identity element with a non-trivial Lie algebra structure, the manifold must globally be a Lie group.
...
\noindent We can specialize Definition \ref{def:lie-homo} to the case of Lie groups.

\begin{definition}[Homomorphism of Lie Groups]\label{def:lie-grhomo}
	Let $G$ be a Lie group as in Definition \ref{def:lie-group}, the map $F: G \to H$ where G and H are Lie groups is called a homomorphism if it is a smooth map and a homomorphism between groups.
\end{definition}

Consider the tangent space of a Lie group. For every $g \in G$, we can define a map $L_g \in C^\infty(G;G)$ by $L_g(h) = gh$ called the \textit{"left multiplication"}. The differential $dL_g$ at a point $h \in G$ is a linear map from $T_hG$ to $T_{gh}G$ as stated in Definition \ref{def:pushforward}. A vector field X on G is called \textit{left-invariant} if
\[
	dL_g (X_h) = X_{gh} \qquad \forall g,h \in G.
\]
It is also true that, for any given vector $v \in T_eG$, there exists a unique smooth left-invariant vector field $X^v$. It can be constructed starting from the vector tangent to the identity by setting $X^v_e = v$. We can then use $L_g$ to define
\[
	X^v_g = dL_g (X^v_e) = dL_g(v).
\]
By construction, this is left-invariant and smooth.
One can show that the commutator of two left-invariant vector fields is, again, a left-invariant vector field. Also, it is worth noting that left-invariant vector fields form a linear subspace of $\mathfrak{X}(G)$ closed respect to the commutator, called a \textit{Lie subalgebra} and thus they inherit the property of a Lie algebra. 

\begin{definition}[Lie algebra of a Lie group]\label{def:lie-gralg}
	Let $G$ be a Lie group, the Lie algebra $\mathfrak{g}$ of $G$ is the tangent space at the identity with the bracket operation defined by
	\[
		[v, w] := [X^v, X^w]_e.
	\] 
\end{definition}
...
The complete proof can be found in [\cite{leeIntroductionSmoothManifolds2013}, Theorem 9.18]. Completeness alone does not yet reveal the full algebraic content of the flow. The following proposition shows that the integral curve of a left-invariant vector field is in fact a group homomorphism.
...
This is proved in \cite[Theorem 20.1]{leeIntroductionSmoothManifolds2013}. These concepts naturally converge to establish a canonical correspondence between a Lie algebra and its associated Lie group. The mathematical tool enacting this exact correspondence is the exponential map.

\begin{definition}[Exponential map]\label{def:exp}
	Let $G$ be a Lie group and let $\gamma_v:\R \to G$ be the integral curve of $X^v$ through $e$. The exponential map of $G$ is
	\[
	\exp: \mathfrak{g} \to G, \qquad \exp(v) := \gamma_v(1).
	\]
\end{definition}

Thanks to the completeness granted by Proposition \ref{prop:complete}, the integral curve $\gamma_v$ is extended globally $\forall t \in \R$. This justifies Definition \ref{def:exp} and it is at the heart of the next properties we discuss.

\begin{proposition}[Properties of exponential map]\label{prop:exp-props}
	Let $G$ be a Lie group, it can be shown that
	\begin{enumerate}
...
This statement is proved in \cite[Proposition 20.8]{leeIntroductionSmoothManifolds2013}. The naturality property is particularly restrictive. It forces the exponential map to commute with any homomorphism of Lie groups. Before exploiting this, it is worth noting a subtlety in the global behaviour of $\exp$.

\begin{remark}
	Global surjectivity of $\exp$ is not automatic in general, it fails ,\textit{e.g.}, for $SL(2,\R)$. Yet it is verified for $G$ compact and connected. This covers the classical groups used in this thesis ($SU(2)$, $U(n)$).
\end{remark}
...
In this section, we start with the general definition of a representation for arbitrary groups, then specialize to Lie groups and Lie algebras, following \cite{hallLieGroupsLie2003}.
...
\begin{definition}[Representation of a group]\label{def:group-rep}
	...
	\[
		\rho : G \to GL(V),
	\]
	where $GL(V)$ denotes the general linear group, the set of all the automorphisms $\psi: V \to V$. A representation is often denoted by $(V, \rho)$.
\end{definition}
...
\begin{remark}
	It is worth noting that this definition is slightly different from the previous one. In the case of a group, the range of the homomorphism $\rho$ is the set of all automorphisms. Instead in Definition \ref{def:lie-rep}, the maps onto $\mathfrak{gl}(V)$ are not all isomorphisms. Observe that $\mathfrak{gl}(V)$ is the \emph{tangent space} to $GL(V)$ at the identity.  Defining an exponential map $g(t) = e^{tX}$ one has $g'(0) = X$, which is generically non-invertible even though every $g(t)$ is invertible. Algebraically, this reflects the fact that a Lie algebra homomorphism $\rho : \mathfrak{g} \to \mathfrak{gl}(V)$ needs only preserve the bracket, $\rho([X,Y]) = [\rho(X),\rho(Y)]$, whereas a group homomorphism $\rho : G \to GL(V)$ must preserve inverses. This forces $\rho(g) \in GL(V)$ for every $g \in G$.
\end{remark}
...
\noindent One of the most relevant properties of a representation is faithfulness.
...
Establishing whether a representation is faithful determines if it captures the full algebraic structure of the original group without redundancy. However, physical systems may realize the identical abstract symmetry through fundamentally different matrix formulations, reflecting a different choice of basis for the underlying state space. Therefore, before attempting to systematically decompose these spaces, it is strictly necessary to formulate a rule to identify when two representations are mathematically equivalent and physically indistinguishable.
...
\begin{remark}
	Another way to express Definition \ref{def:rep-equiv} is the following. In order to define a representation equivalent to $(V, \rho)$ on a $\K$-vector space W, given a vector space isomorphism $T$ we define the homomorphism of the equivalent representation as
	\[
...
\begin{example}[Schur's Lemma on $\mathfrak{su}(2)$]
	...
	\noindent In this simple case, we see how Schur's Lemma guarantees that $\hat J^2$ takes one fixed value on the whole representation, not just on individual states.
\end{example}
...
In the previous sections, we established the formalism of differential geometry and the theory of Lie groups, which are essential tools for describing the global symmetries and the topology of physical systems. However, the configuration manifold turns out to be insufficient. Dynamical evolution naturally arises from the phase space, mathematically identified with the cotangent space of the base manifold. This structure is complete enough to yield a way to compute an isomorphism between covectors and vector fields. The framework provided is called symplectic geometry. 
...
\begin{definition}[Non-degenerate 2-form on a manifold]\label{def:non-deg}
	Let $M$ be a smooth manifold and $\omega \in \Omega^2(M)$. We say that $\omega$ is \emph{non-degenerate} if, for every $p \in M$, the bilinear form $\omega_p$ is non-degenerate on $T_pM$: for every $X \in T_pM$, if $\forall Y \in T_pM$ 
	\[
		\omega_p(X,Y) = 0 \implies X = 0.
	\]
\end{definition}
...
\begin{proposition}\label{prop:skew-normal-form-manifold}
	Let $\omega \in \Omega^2(M)$ on a smooth manifold $M$, and for $p \in M$ set $r_p := \mathrm{rank}(\omega_p)$. Then for every $p \in M$ there exists an integer $n_p$ with $r_p = 2n_p$, and an ordered basis $\hat e = (e_i)$ of $T_pM$, with dual ordered basis $(\alpha^i)$ as per Remark \ref{def:dual-basis}, such that
	\[
...
\noindent A proof of this statement can be found in \cite[Proposition 3.1.2]{abrahamFoundationsMechanics2008}.

\begin{remark}
	Observe that $(e_i)_{i=1,...,2n}$ is a basis of $T_pM$ alone, chosen independently at each $p$, with no claim that $p \mapsto e_i(p)$ varies smoothly, or even continuously.
\end{remark}

\noindent Specializing the previous proposition to the non-degenerate case removes the degenerate block entirely and it fixes the rank at every point, with the following immediate consequences.

\begin{corollary}\label{cor:omega-prop}
	Suppose $\omega$ is non-degenerate by Definition \ref{def:non-deg}. Then $r_p = \dim T_pM =: d$ for every $p \in M$, and $n_p \equiv n$ is the \emph{same} integer at every point. In particular:
	\begin{itemize}
		\item $d$ must be even, so a skew-symmetric non-degenerate $2$-form can exist only on an even-dimensional manifold.
		\item we can define $\omega^n := \omega \wedge \cdots \wedge \omega$ with $n=d/2$ factors, which is nowhere-vanishing and, in the basis used above, it can be written as $\omega_p^{\,n} = n!\,\alpha^1\wedge\alpha^{n+1}\wedge\cdots\wedge\alpha^n\wedge\alpha^{2n} \neq 0$. So, $\omega^n$ is a volume form on $M$, that can be normalized as
		\[
...
This is proved in \cite[Proposition 3.1.5]{abrahamFoundationsMechanics2008}. Beyond restricting the dimension and providing a canonical volume measure, the non-degeneracy of $\omega$ plays a far more active role in the formulation of mechanics. Specifically, a non-degenerate bilinear form naturally induces a point-wise vector space isomorphism between the tangent and cotangent spaces. This duality is the fundamental mathematical machinery required to translate the differential of an observable into a unique vector field that generates the dynamics.

\begin{definition}[Musical isomorphisms]\label{def:musical}
	Let M be a smooth manifold and $\omega \in \Omega^2(M)$ be non-degenerate by Definition \ref{def:non-deg}. We define $\flat :\mathfrak{X}(M) \to \Omega^1(M)$ so that a vector field $$X \mapsto X^\flat = \iota_X\omega = \omega(X, \cdot).$$ 
	
	The inverse map is $\sharp: \Omega^1(M) \to \mathfrak{X}(M)$ so that $\alpha \mapsto \alpha^\sharp$ where $\alpha^\sharp$ is the only vector field $X$ such that $\iota_X \omega= \alpha$.
\end{definition}

\begin{remark}
	Note that the inverse map $\sharp$ is well-defined because $\flat$ is bijective being $\omega$ non-degenerate as in Definition \ref{def:non-deg}. We can infer that $(X^\flat)^\sharp = X$ and $(\alpha^\sharp)^\flat = \alpha$ due to the non-degeneracy of $\omega$.
\end{remark}
...
\begin{theorem}[Darboux]\label{thm:darboux}
	Suppose $\omega$ is a non-degenerate two-form on a 2n-manifold M by Definition \ref{def:non-deg}. Then $\omega$ is closed, meaning $d \omega = 0$, where $d$ denotes the exterior derivative, if and only if there exists a chart $(U, \phi)$ at each $m \in M$ such that $\phi(m) = \boldsymbol{0}$ and $\phi(u) = (x^1(u),..,x^n(u), y^1(u),...,y^n(u))$ with
	\[
		\omega|_U = \sum_{i=1}^n dx^i \wedge dy^i.
	\]
\end{theorem}

\noindent We will not show the proof of this theorem. We refer the reader to \cite[Theorem 3.2.2]{abrahamFoundationsMechanics2008}.

\begin{remark}\label{rem:darboux-consequences}
	One direction of the equivalence descend by direct inspection. If we start from the fact that the canonical local expression of the form is exact, then it is automatically closed. The true analytical depth of Darboux's Theorem, however, lies in its sufficiency. It proves that the closure of $\omega$ is the only requirement needed to guarantee the existence of a local canonical frame. Consequently, any two symplectic manifolds of the same dimension are locally diffeomorphic.
\end{remark}

\noindent Finally, we can state the next important definition that encodes all the structure to derive Hamiltonian mechanics in a natural way.

\begin{definition}[Symplectic manifold]\label{def:sympl-man}
	A symplectic form $\omega$ on a smooth manifold M is a non-degenerate, closed, two-form on M. A symplectic manifold $(M, \omega)$ is a smooth manifold M together with a symplectic form $\omega$ on M. The charts as per the Darboux Theorem \ref{thm:darboux} are called symplectic charts and the associated coordinates are called \emph{canonical}.
\end{definition}
...
\begin{definition}[Canonical transformation]\label{def:canonical-transformation}
	Let $(M,\omega)$ and $(N,\rho)$ be symplectic manifolds. Then $F \in C^\infty(M;N)$ is called \emph{symplectic} or \emph{canonical transformation} if $F^*\rho = \omega$.
\end{definition}
...
This statement is proved in \cite[Proposition 3.2.6]{abrahamFoundationsMechanics2008}. The next theorem is notationally dense, but it is central to this introduction to symplectic geometry. The problem we are facing is the following. Constructing a 1-form on a manifold M, requires a rule that assigns a real number to any tangent vector. When M is already a cotangent space of another manifold Q, $M = T^*Q$, as in the case of the phase space in Hamiltonian mechanics, there is a natural way to accomplish this.

\medskip 
Fixing a $q \in Q$, from the perspective of Q, every point in M is a 1-form, which we denote as $\alpha_q$. From the perspective of M, we can instead consider the tangent vector at the point $\alpha_q$, calling it $w_{\alpha_q}$. 
At this point, the natural way to create a 1-form is to use $\alpha_q$ viewed as a form acting on tangent vectors in $TQ$. Thus, we can apply a pushforward to $w_{\alpha_q}$ in order to move it to the tangent space of Q, pairing it with $\alpha_q$. This is how the \textit{canonical form} $\theta_0$ works.
...
\noindent This theorem is proved in \cite[Theorem 3.2.10]{abrahamFoundationsMechanics2008}.

\begin{remark}
	The superscript $*$ in $\tau^*_Q$ labels it as the \emph{cotangent}-space projection, as opposed to $\tau_Q: TQ \to Q$. It should not be confused with the pullback operator. 
\end{remark}
...
\noindent The full construction of the canonical forms in local coordinates can be found in \cite[p.179]{abrahamFoundationsMechanics2008}. We can also prove that these canonical forms are not changed by any diffeomorphism on the manifold.
...
\noindent We will not prove this theorem, instead, we refer the reader to \cite[Theorem 3.2.12]{abrahamFoundationsMechanics2008}.
