...
\begin{theorem}[Lie Algebra Homomorphism of Hamiltonian Vector Fields]\label{thm:ham-homomorphism}
	Let $(M, \omega)$ be a symplectic manifold. The mapping $\mathcal{H}: C^\infty(M) \to \mathfrak{X}(M)$ defined by $f \mapsto \mathcal{H}_f = -X_f$, where $\iota_{\mathcal{H}_f}\omega = -df$, relates the Poisson algebra of smooth functions to the Lie algebra of vector fields via the identity:
	\[
